...
export async function purgePresentationDataForChannelReset(channelId, context = {}) {
  ...
  const result = await clearPresentationDataForChannel(channelId).catch((err) => {
    console.error(`[Péché Mignon] Purge présentations salon ${channelId} échouée:`, err?.message || err);
    return null;
  });
  ...
  if (messages > 0 || drafts > 0) {
    ...
    console.log(
      `[Péché Mignon] Salon présentation reset (${channelId})${extra} → ${messages} publication(s) et ${drafts} brouillon(s) retirés de la base.`
    );
  }
  return result;
}
